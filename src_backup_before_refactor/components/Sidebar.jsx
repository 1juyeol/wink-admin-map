...
import {
    ChevronRight, ChevronDown, Star, Folder, FileText, PanelLeftClose, PanelLeftOpen,
    FolderOpen, FolderClosed, LayoutGrid, Users, CheckSquare, Layers, PanelLeft
} from 'lucide-react';
...
export const Sidebar = React.memo(({
    ...
    starredMenus = [],
    onToggleStarMenu,
    isRefreshing, // [v6.11.23] 초기화 리프레시 상태 추가
    isAdmin
}) => {
    ...
    const renderTree = (nodes, depth = 0) => {
        return nodes.map((node) => {
            ...
            const hasAccess = (() => {
                ...
                const check = (n) => {
                    if (n.items?.some(i => selectedTeams.some(t => permissions[t]?.[i.id]))) return true;
                    ...
                    >
                        <div className="flex items-center gap-4 flex-1 min-w-0 transition-all">
                            ...
                            <div className="flex items-center gap-1.5 ml-2 shrink-0 pointer-events-auto">
                                {isCompareMode && selectedTeams.length > 0 && (
