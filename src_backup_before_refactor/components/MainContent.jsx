...
import {
    CheckCircle, Lock, UserCheck, MousePointerClick,
    ChevronDown, ChevronUp, XCircle, GitCompare, Users, Star
} from 'lucide-react';
import { cn } from '../utils/cn';
import { getDesignByType } from '../utils/designSystem';

export const MainContent = React.memo(({ activeNode, selectedTeams, allData, starredMenus = [], onToggleStarMenu, isCompareMode = false, searchTargetId, isAdmin }) => {
    const isComparison = isCompareMode && selectedTeams.length > 1;
    ...
    const hasPageAccess = (() => {
        if (!selectedTeams || selectedTeams.length === 0) return true;

        const checkNode = (node) => {
            if (!node) return false;
            // 1. 현재 노드의 아이템(버튼/뷰 등) 중 권한이 있는지 확인
            if (node.items?.some(item =>
                selectedTeams.some(team => allData.permissionMap[team]?.[item.id] === true)
            )) return true;
            ...
            {selectedTeams.length > 1 && (
                <div className={cn(
                    ...
                    {isComparison ? (
                        ...
                        <>
                            <Users size={16} />
                            <span><strong>통합 모드:</strong> 선택된 {selectedTeams.length}개 팀 중 <strong>하나라도</strong> 권한이 있으면 접근 가능한 것으로 표시됩니다. (겸직자 기준 합집합)</span>
                        </>
                    )}
                </div>
            )}
                        ...
                        {(() => {
                            ...
                            return finalFeatures.map((group, index) => {
                                if (group.type === 'single') {
                                    const { item } = group;
                                    const isAllowed = selectedTeams.length === 0
                                        ? true
                                        : selectedTeams.some(team => allData.permissionMap[team]?.[item.id]);
                                    ...
                                    return (
                                        <React.Fragment key={`grp-${group.name}-${index}`}>
                                            ...
                                            <tr className="border-t-4 border-slate-400 dark:border-slate-500 shadow-sm bg-white dark:bg-slate-900">
                                                <td colSpan={5 + Math.max(1, selectedTeams.length)} className="py-5 px-6">
