    ...
    try {
        ...
        rawRows.forEach((row, index) => {
            ...
            const pathNodes = [
                ...
                // [v6.8.6] LCMS 오늘의 공부 계층 구현을 위한 추가 페이지 계층
            ...
            if (functionName && !strategy.shouldFilterOut(functionName, rDesc, current.name)) {
                // [v8.9] 사용자 요청: 특정 복합 명칭들을 개별 영역(항목)으로 분리하여 생성
                // .includes()를 사용하여 대괄호나 추가 텍스트가 있어도 매핑되도록 개선
                const namesToSplit = functionName.includes('재회수')
                    ? ['재회수 예약', '재회수 예약 변경', '재회수 예약 취소']
                    : (functionName.includes('반복수업'))
                        ? ['반복수업', '반복수업 삭제', '유료회원 체험수업 등록']
                        : [functionName];

                namesToSplit.forEach((name, splitIdx) => {
                    // [v12.1] 중복 키 경고 해결: 분리된 항목들에 대해 유니크한 ID 생성 (RowIndex-Suffix)
                    current.items.push({
                        id: namesToSplit.length > 1 ? `${excelRowIndex}-${splitIdx}` : excelRowIndex,
                        name: name,
        ...
        const sortedTeams = Object.keys(permissionMap).sort((a, b) => {
            const countA = Object.values(permissionMap[a]).filter(val => val === true).length;
            const countB = Object.values(permissionMap[b]).filter(val => val === true).length;
