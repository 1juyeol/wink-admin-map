...
export const HiddenMenuModal = ({ isOpen, onClose, teams = [] }) => {
    const { data, findNodeByName } = usePermission();
        ...
        lines.forEach(line => {
            ...
            if (funcMatch) {
                funcMatch.forEach(f => {
                    ...
                    // [v12.2] 재회수/반복수업 강제 split 로직 제거
                    // 엑셀에 이미 개별 행으로 분리되어 있으므로 그대로 1:1 사용
    ...
    // [v12.0] 부가기능 모달 한정으로 L열(O 권한)이 많은 순서대로 재정렬
    const sortedTeamsByPermissions = useMemo(() => {
        if (!data?.permissionMap) return teams;

        return [...teams].sort((a, b) => {
            const countA = Object.values(data.permissionMap[a] || {}).filter(val => val === true).length;
            const countB = Object.values(data.permissionMap[b] || {}).filter(val => val === true).length;
            return countB - countA;
        });
    }, [teams, data?.permissionMap]);

    if (!isOpen) return null;

    const sortedRelevantTeams = sortedTeamsByPermissions.filter(t => groupedData[t]);
            ...
            <div className="relative w-full max-w-5xl h-full max-h-[85vh] bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300">
                ...
                <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 sticky top-0 z-10">
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-amber-50 dark:bg-amber-900/30 rounded-2xl text-amber-600 dark:text-amber-400 shadow-sm border border-amber-100 dark:border-amber-900/50">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-bold mt-1">
                                중복 권한이 부여될 경우 예상치 못하게 권한이 제한되는 메뉴와 기능을 분석합니다.
                            </p>
                        </div>
                    </div>
                    <button
                                                    ...
                                                    <div key={conflict} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                            {functions.map((func, fidx) => {
                                                                // Extract the raw text before the parentheses (e.g. "결제 카드 변경(CHANGE_PAYMENT_CARD)" -> "결제 카드 변경")
                                                                // Also handle "재회수(RETURN_DELIVERY 드롭다운)"
                                                                let cleanName = func.split('(')[0].trim();
                                                                cleanName = cleanName.replace(/드롭다운.*$/, '').trim(); // Remove lingering "드롭다운" if it was outside parentheses, etc

                                                                const node = typeof findNodeByName === 'function' ? findNodeByName(cleanName) : null;
                                                                const type = node?.type || 'page'; // default to page if not found
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border", design.className)}>
                                                                            {design.icon(16, 2)}
                                                                        </div>
                                                                        <div className="min-w-0 flex-1">
                                                                            <div className="flex items-center gap-2 mb-0.5">
                                                                                <span className={cn(
                                                                                    "text-[14px] font-black truncate flex-1 min-w-0",
                                                                                    (type === 'button' || type === 'dropdown') ? "text-sky-800 dark:text-sky-200" : "text-slate-700 dark:text-slate-200"
                                                                                )}>
                                                                                    {func}
                                                                                </span>
                                                                                {node?.parentName && (
                                                                                    <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 font-bold shadow-sm shrink-0 whitespace-nowrap">
                                                                                        {node.parentName}
                                                                                    </span>
                                                                                )}
                                                                            </div>
